...
void showfmta(long addr, wordtype word) {
  ...
  switch (word.fmta.op) {
    ...
    case cgt:
    case cge:
      printf("%5ld %-6s   r%d, r%d, r%d", addr, opcode, word.fmta.ri,
...
void showfmtb(long addr, wordtype word) {
  ...
  switch (word.fmtb.op) {
    ...
    case lw:
    case lb:
      printf("%5ld %-6s   r%d, %d(r%d)", addr, opcode, word.fmtb.ri,
    ...
    case sw:
    case sb:
      printf("%5ld %-6s   %d(r%d), r%d", addr, opcode, word.fmtb.k,
    ...
    case cgti:
    case cgei:
      printf("%5ld %-6s   r%d, r%d, %d", addr, opcode, word.fmtb.ri,
...
short issymchar(char c) { return (isalnum(c) || c == '_'); }
...
void showusage() {
  ...
  printf(
      "The command line may contain source file names and options in any "
      "order.\n");
  printf(
      "There should be at least one source file.  Source files will be "
      "loaded\n");
  ...
  printf(
      "       Use +o or -o followed by a name to changes the list file "
      "name.\n");
}
